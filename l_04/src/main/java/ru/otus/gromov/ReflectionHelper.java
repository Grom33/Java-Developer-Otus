...
public class ReflectionHelper {
    ...
    static Object getAnnotationsMap(Class clazz) {
        Map<Annotation, List<Method>> annotationsMap = new HashMap<>();
        Method[] methods = clazz.getDeclaredMethods();
        for (Method method : methods) {
            Annotation annotation[] = method.getAnnotations();
            for (Annotation anno : annotation) {
                if (annotationsMap.containsKey(anno)) {
                    annotationsMap.get(anno).add(method);
                } else {
                    annotationsMap.put(anno, new ArrayList<>(Arrays.asList(method)));
                }
            }
        }
